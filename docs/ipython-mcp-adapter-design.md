...
本文档是 IPython MCP adapter 的运行时与生命周期事实源。工具返回的 pretty text、`content` / `structuredContent` 双通道规则、output log handle、行范围读取、搜索和大输出省略规则，以 `docs/ipython-mcp-output-surface-design.md` 为当前事实源。本文档中的输出示例只表达运行时语义，不重复定义展示面。
...
**output log** 指 execution 输出缓存的日志化文本视图。当前实现包含 combined、stdout、stderr、result 和 traceback 五个 stream。`python-output:<execution_id>` 是 combined log handle；固定后缀 `/stdout`、`/stderr`、`/result`、`/traceback` 定位分流日志。
...
工具集合不包含 `start_python`。设置 workspace 本身即启动 kernel。`run_python` 与 `wait_python` 不包含输出截断参数；输出缩小、按行读取和搜索由日志工具承担。

`run_python` 的 MCP tool schema 暴露唯一字符串字段 `freeform`。`freeform` 是原始 Python cell 文本。单次等待上限默认 10 秒；如需覆盖本次等待上限，cell 中只能使用 `docs/ipython-mcp-freeform-run-python-design.md` 定义的 canonical directive：`# loommux: timeout_seconds=120`。`wait_python` 仍保持结构化参数，默认等待 30 秒。
...
run_python(freeform="x = 41")
run_python(freeform="x + 1")
```

长任务使用：

```text
run_python(freeform="# loommux: timeout_seconds=30\n长时间运行的代码") -> status = running, execution_id = "exec-0001"
read_python_output(execution_id="exec-0001", line_range="-20:")
search_python_output("error|warning", execution_id="exec-0001", query_mode="regex")
...
适配层不丢弃已收集的 stdout、stderr、result text 或 traceback。`run_python` 与 `wait_python` 的默认返回是否携带输出正文，由 output surface 规则决定：小输出可直接返回；running 或大输出返回 output log handle，并通过日志工具继续读取。
...
```text
run_python(freeform: str)
```

输入规则：

1. `freeform` 必须是字符串。
2. `freeform` 是原始 Python cell 文本，空字符串允许提交，由 IPython kernel 决定执行结果。
3. `run_python` MCP schema 只包含 `freeform`，不包含 `code`、`timeout_seconds` 或输出截断参数。
4. 当前工具调用默认等待 10 秒。
5. 若整个 `freeform` 文本中恰好一行完整匹配 `# loommux: timeout_seconds=<positive decimal number>`，本次调用使用该秒值作为等待上限。
6. 无匹配、多匹配或无效 loommux 注释时，使用默认 10 秒，不产生 parser failure。
7. timeout directive 保留在提交给 kernel 的原始 cell 中，不创建、读取或更新 `LOOMMUX_*` 运行时变量。
...
4. 如果 kernel 空闲，创建 execution 并提交原始 `freeform` 文本。
5. 当前工具调用等待最多 10 秒，或等待唯一有效 timeout directive 指定的秒数。
...
2. 如果未传入 `output_log` 但传入 `execution_id`，读取该 execution 的 selected stream。
3. 如果两者都未传入且存在 current execution，读取 current execution 的 selected stream。
4. 如果不存在 current execution，读取 last execution 的 selected stream。
5. 如果没有可读取 execution，返回 `execution_not_found`。
6. 如果 `output_log` 已包含 stream 后缀，该后缀决定 stream；显式传入冲突 stream 返回 `invalid_output_log`。
...
```json
{
  "ok": true,
  "execution_id": "exec-000002",
  "output_log": "python-output:exec-000002/stdout",
  "stream": "stdout",
  "line_range": "-10:",
  ...
  "omitted_before": 90,
  "omitted_after": 0,
  "text": "91 | partial output"
}
```

### 4.8 `search_python_output`
...
选择规则与 `read_python_output` 相同。`literal` 按字面量搜索；`regex` 按正则搜索；`auto` 优先按正则解释，正则编译失败时回退为字面量搜索。上下文参数必须大于等于 0。
...
### 4.9 `python_execution_status`

工具定义：

```text
python_execution_status(execution_id: str | null = null)
```

选择规则与 `read_python_output` 的 execution 选择部分相同：显式 execution 优先，其次 current execution，再其次 last execution。

该工具返回 execution 的结构化状态，不返回完整日志正文。状态返回包含 canonical `output_log`，分流 handle 由固定后缀派生。
...
execution 选择规则与 `python_execution_status` 相同。
...
返回形状与 `run_python` execution status 一致，并附带 kernel 状态：
...
2. 调用 `run_python(freeform="x = 41")`。
3. 调用 `run_python(freeform="x + 1")`。
...
1. 调用 `set_workspace(valid_workspace)`。
2. 调用 `run_python(freeform="print('hello')")`。
...
1. 调用 `set_workspace(valid_workspace)`。
2. 调用 `run_python(freeform="import sys; print('bad', file=sys.stderr)")`。
...
1. 调用 `set_workspace(valid_workspace)`。
2. 调用 `run_python(freeform="# loommux: timeout_seconds=0.5\\nimport time\\ntime.sleep(3)\\n42")`。
...
1. 调用 `set_workspace(valid_workspace)`。
2. 调用 `run_python(freeform="# loommux: timeout_seconds=0.5\\nimport time\\nfor i in range(3):\\n    print(i, flush=True)\\n    time.sleep(1)")`。
3. 调用 `read_python_output(execution_id=execution_id, stream="stdout")`。

期望：

1. 返回 `ok == true`。
2. 返回 `execution_id` 与 stdout `output_log`。
3. `text` 包含至少一个已打印数字，除非 execution 在第一次 sleep 前尚未产生输出。
4. 后续 `wait_python` 完成后，再读取 stdout log，`text` 包含 `0`、`1`、`2`。
...
1. 调用 `set_workspace(valid_workspace)`。
2. 调用一个会运行数秒且包含 `# loommux: timeout_seconds=0.5` directive 的 `run_python(freeform=...)`。
3. 在其 running 时再次调用 `run_python(freeform="123")`。
...
6. 调用 `run_python(freeform="'x' in globals()")`。
...
3. 调用 `reset_python()`。
4. 调用 `python_execution_status(execution_id)`。
...
1. `run_python` 只包含 `freeform` 输入字段。
2. `freeform` 类型为 string，且 `freeform` 是 required。
3. `run_python` 没有 `code`。
4. `run_python` 没有 `timeout_seconds`。
5. `run_python` 没有 `max_output_chars`。
6. `run_python` 没有其它截断参数。
...
1. 文档中的九个工具均存在，且不存在 `start_python`。
...
5. `run_python` 保留状态。
6. `run_python` 默认等待 10 秒。
7. `run_python` 的唯一单次等待覆盖方式是 canonical directive `# loommux: timeout_seconds=120`。
8. `run_python` 超时返回 running，不自动中断。
9. running execution 可通过 `read_python_output` 按 stream 与行范围查看输出。
10. output log 可通过 `search_python_output` 执行字面量或正则搜索。
11. execution 结构化状态可通过 `python_execution_status` 查询，且不返回完整日志正文。
12. running execution 可通过 `wait_python` 等待。
13. busy 时新 `run_python` 不排队。
14. `interrupt_python` 只发送中断，不清空状态。
15. `reset_python` 杀掉 kernel 并启动新 kernel。
16. reset 后旧变量不可见。
17. stdio 客户端不导入 server。
18. 全部黑盒测试通过。
...
开发者不得将 MCP `run_python` tool schema 改回 `code` / `timeout_seconds` 结构化输入。

开发者不得为 timeout directive 增加 alternate syntax、宽松匹配或 parser warning surface。
