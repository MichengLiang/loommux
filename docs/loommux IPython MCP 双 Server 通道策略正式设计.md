...
这个设计本身成立，并且对合规客户端是正确的。问题来自一类客户端的实际行为：它们不按照“模型读 content、程序读 structuredContent”的分工使用 MCP result，而是默认或优先读取 `structuredContent`，甚至把结构化对象注入模型上下文。这样一来，`presentation.py` 中对模型阅读面的优化会被绕过。模型看到的不是 output-first、footer 明确、日志句柄清晰的 pretty text，而是 raw dict、空字段、状态字段和机器对象。
...
**pretty text** 指 presentation layer 生成的文本 content，例如 Python 输出正文、footer、日志读取结果、搜索命中或 compact status。
...
`execution_id`、`status`、`output_log` 已经在 content footer 中出现。content-only server 的原则是让模型从 pretty text 读取这些信息。它不是程序 API。需要程序 API 的客户端应使用标准 server。
...
`run_python` 和 `wait_python` 对小完成输出直接展示 combined output body，再给一行 footer。这样模型先看到 Python 可见输出，而不是状态字段。

running execution 不展示 partial output body，只给省略 footer 和 output log handle。这样模型知道应该继续 wait、read 或 search。

大输出不塞满工具返回，只给 line count、limit 和 output log handle。这样模型知道应该通过日志工具分段读取。

`read_python_output` 直接展示读取到的日志文本，再给一行 footer。它不是状态快照。

`search_python_output` 直接展示命中和上下文，再给一行 footer。无命中也用 compact footer 表达。
...
期望：content 文本是 running omitted footer；`structured_content is None`，`data is None`。
