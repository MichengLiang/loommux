...
**execution status** 是一次 `run_python` 提交的结构化状态对象。它回答 execution 是否存在、是否正在运行、是否完成、是否出错、由哪个 kernel 执行、有哪些 output log handle 可供读取。
...
`run_python` 与 `wait_python` 返回 execution status，并在返回体中包含 output log handles。它们不提供行范围、grep 或 max chars 参数。读取日志由日志工具完成。

默认保护规则：`run_python` 与 `wait_python` 的状态返回在兼容期仍可携带 `stdout`、`stderr` 与 `result_text` 字段；只有 execution 已结束且 combined output log 不超过 300 行时，状态返回才携带完整日志正文。execution 仍为 `running`，或 combined output log 超过 300 行时，这些文本字段必须为空，返回体只保留 execution id、状态、错误摘要、output log handles 和省略元数据。省略元数据包括 `output_omitted`、`output_omitted_reason`、`output_line_limit` 与 `output_total_lines`。完整日志通过 `read_python_output` 或 `search_python_output` 读取。
...
```text
read_python_output(execution_id=None, output_log=None, line_range=None, show_line_numbers=False, max_chars=None)
search_python_output(query, execution_id=None, output_log=None, query_mode="auto", context_before=0, context_after=0, ignore_case=False, max_chars=None)
```
...
2. 如果未传入 `output_log` 但传入 `execution_id`，选择该 execution 的 combined log。
3. 如果两者都未传入，选择 current execution 的 combined log。
4. 如果不存在 current execution，选择 last execution 的 combined log。
...
```json
{
  ...
  "logs": {
    "combined": "python-output:exec-000001",
    "stdout": "python-output:exec-000001/stdout",
    "stderr": "python-output:exec-000001/stderr",
    "result": "python-output:exec-000001/result",
    "traceback": "python-output:exec-000001/traceback"
  },
  "error": null,
  "kernel": {
    "busy": true,
...
短期可以保留 `Execution.snapshot()` 中的 `stdout`、`stderr`、`result_text` 字段，以降低迁移成本。但新的工具文档与测试应以 log handle 为主。长期目标是让状态返回不携带完整日志正文。
...
1. `run_python` 返回 `output_log` 与 `logs`。
2. `wait_python` 返回 `output_log` 与 `logs`。
