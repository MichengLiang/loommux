...
第一阶段交付一个可由 MCP host 通过 stdio 启动的 `loommux-pueue` Rust binary。一个 binary process 服务一个 MCP client，维护一个 adapter-local execution namespace，并通过 `pueue-lib` 连接已经运行的 Pueue `v4.0.4` daemon。agent 可以提交 shell script，观察多个并发 execution，读取和搜索保留日志，执行有界等待，并对最近观察为 queued/stashed 或 running/paused 的 task 发起状态敏感的定向控制。

交付包包括：

* `engines/pueue/` Rust crate 与锁定依赖；
* `run_shell`、`status`、`execution_status`、`read_output`、`search_output`、`wait` 和 `cancel` 七个 MCP tools；
* Pueue 声明式 workspace config，并保留既有 Python engine 的 resolver contract；
* 与 Pueue `v4.0.4` / `pueue-lib 0.31.1` 配对的 typed gateway；
...
. Pueue `4.0.4` CLI 与 daemon 可在本机运行；
. Pueue `v4.0.4` tag 和 `pueue-lib 0.31.1` source 可定位；
. 与 Pueue target 相容的 Rust stable toolchain（最低 `1.88`）、`cargo-nextest`、`cargo-llvm-cov`、Clippy 和 rustfmt 可用；
...
|cross-engine canonical workspace contract suite 与 `VER-040`。

|ART-04
|Pueue typed gateway 与 isolated daemon harness。
|Pueue `v4.0.4` integration suite。

|ART-05
|execution record、state projection 和七工具控制闭环。
|design verification matrix `VER-001` 至 `VER-017`、`VER-036` 至 `VER-039`。
...
|完整 `VER-001` 至 `VER-041` traceability report 与 quality gate manifest。
...
|G1
|工程与握手
|正式 Rust crate locked build 成功；真实 isolated `pueued v4.0.4` handshake 和 `Status` exchange 通过。
...
Inputs::
原始 Pueue 讨论材料、相邻纯终端调研、现有 Loommux 代码/测试/设计文档、Pueue `v4.0.4` source、目标 `rmcp` source 与本机 probe。
...
* 明确 Pueue version pairing、task state、wait、log、kill/remove、working-directory canonicalization、process wrapper 和 connection 行为；
...
* exact pin `pueue-lib = "=0.31.1"` 和 `rmcp = "=3.0.0-beta.2"`；
...
Purpose::
建立 Pueue engine 的声明式 workspace contract，并保持它与既有 IPython engine 相同的 workspace resolution 观察分类。
...
Dependencies::
WP-00；Pueue Rust implementation 依赖 WP-01。

Outputs::
versioned TOML schema、Rust resolver、Pueue generic/Codex examples、workspace fixtures 和 startup error taxonomy。
...
* 保留 Python/IPython engine 的 Python callback resolver，不向该 runtime 增加 TOML 配置依赖；
* 为两个 engine 分别保留 Python 与 TOML examples，使 Codex marker 规则都显式由 host 配置授权；
* 保证 workspace 只在 startup 解析一次；
* 对 resolver 最终候选目录执行 filesystem canonicalization，并将 canonical absolute path 传给 status 与每个 `AddRequest.path`；
* 将 missing、not-directory 与 canonicalization I/O failure 分类为不同 startup error；
* 在两个 engine 的 status 中使用 `launch_cwd` / `explicit_config` 相同观察分类；
* 为 Rust resolver 建立路径、symlink、broken symlink、permission failure、root、marker missing、file/directory mismatch 和 malformed TOML tests。

Acceptance evidence::

* Rust fixture 覆盖 canonical path 和 resolution，Python resolver tests 保持既有 callback contract；
* 配置错误在 tool ready 前失败且不 silent fallback；
* Pueue integration test 证明 `AddRequest.path` 等于 canonical resolved workspace；
* repository 文档明确 Python/IPython 使用 Python resolver，而 Rust/Pueue 使用 TOML。

Not done when::
任一 engine 猜测配置 grammar、Rust 自动扫描 `.git`/`.codex`、工具允许逐次 cwd override、symlink path 未规范化，或错误配置静默使用 launch cwd。
...
Inputs::
xref:design-specification.adoc#pueue-gateway[Pueue Gateway 与协议边界]、Pueue `v4.0.4` source 与 crate pairing evidence。
...
Outputs::
`PueueMCPAdapter` core、`PueueExecution`、sequence/map/recent/active logic、submission atomicity、task identity fingerprint、terminal freeze、state/result projection 和 fake-gateway component suite。
...
* 保存 canonical task identity fingerprint，排除 Pueue start 时注入的 runtime environment 字段；
* 在每次 nonterminal observation 中先验证 fingerprint，再投影状态或追加日志；
* 冻结已公开 terminal record，不因同一 task ID 被外部 in-place restart 而重新打开；
...
* `VER-001` 至 `VER-008`、`VER-017`、`VER-036` 至 `VER-039` 通过；
...
Not done when::
task ID 进入 public schema、rejected Add 消耗编号、使用 scalar current execution、unknown task 被猜测为 killed、external restart 重新打开 terminal record，或状态映射散落在多个 tool handler。
...
* 验证可表示为非零 `Duration` 与 monotonic deadline 的 positive finite decimal，以及 unknown、duplicate 和 missing values；
...
* 观察外部 task-specific start 造成的 stashed-to-running 或 stashed-to-failed 转换；
* 对最近观察为 queued/stashed 的 task 使用 task-specific Remove；明确 success 时建立 local cancelled 控制终态，并保持 `output_complete = false`；
* 对 running/paused 使用 task-specific Kill，区分 request accepted 与 later killed；
* 对 locked、terminal、backend missing、stashed force-start/remove race 和其他 refresh/control race 返回规定结果，不从 Remove success 推断未观察到的 backend result；
...
* `VER-009` 至 `VER-017` 与 `VER-039` 全部通过；
...
Not done when::
wait deadline 会 kill task、wait 持锁 sleep、queued task 无法发起删除控制、Remove success 被解释为“task 从未启动”、kill success 被直接标记 killed、control request 自动重放，或 cancel 可能影响同 group 的其他 task。
...
* 为 `VER-001` 至 `VER-041` 建立唯一 automated test mapping；
...
* 使用独立 Pueue client 驱动 stashed force-start、pause 与 resume，并通过 Loommux cancel 驱动 task-specific kill；
* 使用记录 descendant PID 的多层进程 fixture，验证 pause/resume 信号作用范围和 kill 后零存活 descendant；
...
* `VER-001` 至 `VER-041` traceability coverage 为 100%；
...
Purpose::
将 Rust engine、Python workspace migration、文档、安装入口和 quality gates 组合成一个无冲突的可交付包。
...
Outputs::
README 与安装说明、engine selection 说明、配置示例、release checklist、最终 verification log、变更记录和 scoped commit。

Tasks::

* 在项目 README 中定义 IPython 与 Pueue engine 的不同执行世界和选择入口；
* 记录 `pueued 4.0.4` 前置条件、binary 启动、result mode 和 workspace config；
...
* repository-wide search 明确区分 Python resolver 与 Pueue TOML，且无公开 task ID、current execution、CLI wrapper 或 adapter-owned daemon 陈述；
* release log 记录工具版本、命令、exit code 和 coverage 数字；
* commit 只包含 Pueue engine、engine-specific workspace contract、直接受影响测试与文档。
...
本节只定义节点之间的验证关系，不复制各节点的规范内容。`VER-*` 的场景和预期结果以 xref:design-specification.adoc#verification-matrix[设计验证矩阵]为准；`WP-*` 是实现与证据的责任单元；`SYS-*` 是发布判定。每个 ID 均显式列出，使单个 ID 可以通过 repository search 定位其上游和下游关系。
...
|===
|Verification |Owner |Related gates |Primary authority

|VER-001 |WP-04、WP-08 |SYS-DOD-003、SYS-DOD-004、SYS-DOD-006、SYS-DOD-012 |xref:design-specification.adoc#execution-record[Execution record]与 xref:design-specification.adoc#automatic-output-delivery[自动输出交付]
|VER-002 |WP-04 |SYS-DOD-004、SYS-DOD-005 |xref:design-specification.adoc#submission-atomicity[接受与绑定原子性]
|VER-003 |WP-04、WP-05 |SYS-DOD-004、SYS-DOD-013 |xref:design-specification.adoc#directive-validation[指令验证规则]
|VER-004 |WP-03、WP-04 |SYS-DOD-005、SYS-DOD-015、SYS-EXC-006 |xref:design-specification.adoc#connection-failures[连接失败与重连]
|VER-005 |WP-04、WP-08 |SYS-DOD-003、SYS-DOD-007 |xref:design-specification.adoc#active-selection[Active 与 recent]
|VER-006 |WP-04 |SYS-DOD-006、SYS-EXC-001、SYS-EXC-003 |xref:design-specification.adoc#state-projection[状态投影]
|VER-007 |WP-04、WP-08 |SYS-DOD-006、SYS-EXC-001 |xref:design-specification.adoc#state-projection[状态投影]
|VER-008 |WP-04 |SYS-DOD-006、SYS-EXC-001、SYS-EXC-010 |xref:design-specification.adoc#state-projection[状态投影]
|VER-009 |WP-06 |SYS-DOD-008 |xref:design-specification.adoc#wait-tool[`wait`]
|VER-010 |WP-06 |SYS-DOD-008、SYS-EXC-001 |xref:design-specification.adoc#wait-tool[`wait`]
|VER-011 |WP-03、WP-06 |SYS-DOD-015、SYS-EXC-004 |xref:design-specification.adoc#gateway-exchange[原子请求响应]
|VER-012 |WP-06、WP-07 |SYS-DOD-009、SYS-DOD-010、SYS-EXC-001、SYS-EXC-010 |xref:design-specification.adoc#cancel-tool[`cancel`]与 xref:design-specification.adoc#output-collector[增量收集]
|VER-013 |WP-06 |SYS-DOD-009、SYS-EXC-001、SYS-EXC-010 |xref:design-specification.adoc#cancel-tool[`cancel`]
|VER-014 |WP-06 |SYS-DOD-009、SYS-EXC-001 |xref:design-specification.adoc#cancel-tool[`cancel`]
|VER-015 |WP-06 |SYS-DOD-009、SYS-EXC-001 |xref:design-specification.adoc#cancel-tool[`cancel`]
|VER-016 |WP-03、WP-06 |SYS-DOD-009、SYS-DOD-015、SYS-EXC-001、SYS-EXC-006 |xref:design-specification.adoc#cancel-tool[`cancel`]与 xref:design-specification.adoc#connection-failures[连接失败与重连]
|VER-017 |WP-04、WP-09 |SYS-DOD-006、SYS-EXC-001、SYS-EXC-010 |xref:design-specification.adoc#mapped-task-mutation[Mapped task 外部变更]
|VER-018 |WP-07 |SYS-DOD-010 |xref:design-specification.adoc#combined-transcript[Combined transcript]
|VER-019 |WP-07 |SYS-DOD-010、SYS-EXC-008 |xref:design-specification.adoc#output-collector[增量收集]
|VER-020 |WP-07 |SYS-DOD-011、SYS-EXC-007 |xref:design-specification.adoc#terminal-normalization[Terminal text normalization]
|VER-021 |WP-07 |SYS-DOD-010、SYS-EXC-001 |xref:design-specification.adoc#output-collector[增量收集]
|VER-022 |WP-07 |SYS-DOD-010 |xref:design-specification.adoc#line-log[Line coordinates]
|VER-023 |WP-07 |SYS-DOD-010 |xref:design-specification.adoc#search-model[Search]
|VER-024 |WP-07、WP-08 |SYS-DOD-012、SYS-EXC-008 |xref:design-specification.adoc#automatic-output-delivery[自动输出交付]
|VER-025 |WP-05、WP-07、WP-08 |SYS-DOD-012、SYS-DOD-013 |xref:design-specification.adoc#automatic-output-delivery[自动输出交付]
|VER-026 |WP-05 |SYS-DOD-013、SYS-EXC-002 |xref:design-specification.adoc#directive-consumption[指令消费与保留]
|VER-027 |WP-05 |SYS-DOD-013、SYS-EXC-002 |xref:design-specification.adoc#directive-surface[Canonical directive surface]
|VER-028 |WP-02 |SYS-DOD-014 |xref:design-specification.adoc#workspace-contract[Workspace contract]
|VER-029 |WP-04、WP-05、WP-08 |SYS-DOD-005、SYS-DOD-013 |xref:design-specification.adoc#task-environment[Task environment]
|VER-030 |WP-03、WP-09 |SYS-DOD-002、SYS-DOD-015、SYS-EXC-010 |xref:design-specification.adoc#stdio-transport[stdio transport]
|VER-031 |WP-03、WP-09 |SYS-DOD-015、SYS-EXC-006、SYS-EXC-010 |xref:design-specification.adoc#connection-failures[连接失败与重连]
|VER-032 |WP-09 |SYS-DOD-016、SYS-EXC-010、SYS-EXC-011 |xref:design-specification.adoc#ownership[对象所有权与生命周期]
|VER-033 |WP-04、WP-09 |SYS-DOD-004、SYS-DOD-005、SYS-EXC-005 |xref:design-specification.adoc#execution-namespace[Adapter-local execution]
|VER-034 |WP-08 |SYS-DOD-003 |xref:design-specification.adoc#result-channels[Content 与 structured content]
|VER-035 |WP-08 |SYS-DOD-005 |xref:design-specification.adoc#sensitive-data[不公开数据]
|VER-036 |WP-04 |SYS-DOD-005、SYS-DOD-006、SYS-EXC-001、SYS-EXC-014 |xref:design-specification.adoc#mapped-task-mutation[Mapped task 外部变更]
|VER-037 |WP-04 |SYS-DOD-005 |xref:design-specification.adoc#task-identity[Daemon-global task ID]
|VER-038 |WP-04 |SYS-DOD-006 |xref:design-specification.adoc#mapped-task-mutation[Mapped task 外部变更]
|VER-039 |WP-04、WP-06 |SYS-DOD-006、SYS-DOD-009、SYS-EXC-001、SYS-EXC-010 |xref:design-specification.adoc#execution-lifecycle[Execution 生命周期]
|VER-040 |WP-02 |SYS-DOD-014 |xref:design-specification.adoc#workspace-contract[Workspace contract]
|VER-041 |WP-06、WP-10 |SYS-DOD-009、SYS-EXC-010、SYS-EXC-011 |xref:design-specification.adoc#derived-constraints[推导约束]
|===
...
|SYS-DOD-001 |WP-01 |locked build、binary artifact 与 startup smoke test。
|SYS-DOD-002 |WP-03 |版本配对审计、真实 handshake 与 VER-030。
|SYS-DOD-003 |WP-08 |tool discovery/schema snapshot、VER-001、VER-005 与 VER-034。
|SYS-DOD-004 |WP-04 |VER-001、VER-002、VER-003 与 VER-033。
|SYS-DOD-005 |WP-04、WP-08 |binding/property tests、VER-002、VER-035、VER-036 与 VER-037。
|SYS-DOD-006 |WP-04 |VER-006、VER-007、VER-008、VER-017、VER-038 与 VER-039。
|SYS-DOD-007 |WP-04、WP-08 |VER-005 与 specific-tool schema inspection。
|SYS-DOD-008 |WP-06 |VER-009 与 VER-010。
|SYS-DOD-009 |WP-06 |VER-012、VER-013、VER-014、VER-015、VER-016、VER-039 与 VER-041。
|SYS-DOD-010 |WP-07 |VER-018、VER-019、VER-021、VER-022 与 VER-023。
|SYS-DOD-011 |WP-07 |VER-020 与跨 chunk normalization contract suite。
|SYS-DOD-012 |WP-07、WP-08 |VER-024 与 VER-025。
|SYS-DOD-013 |WP-05 |VER-003、VER-026 与 VER-027。
|SYS-DOD-014 |WP-02 |跨 engine fixtures、VER-028 与 VER-040。
|SYS-DOD-015 |WP-03、WP-06 |VER-004、VER-011、VER-016 与 VER-031。
|SYS-DOD-016 |WP-09 |VER-032 与 shutdown request audit。
|SYS-DOD-017 |WP-10 |本节 41 个 verification mapping 的 automated-test traceability report。
|SYS-DOD-018 |WP-10 |line、function、region coverage report 与 mapping gap audit。
|SYS-DOD-019 |WP-01、WP-11 |versioned quality entrypoint 的全部 exit status 与 `git diff --check`。
|SYS-DOD-020 |WP-00、WP-11 |文档结构审计、stale-claim scan 与 release review。
|SYS-EXC-001 |WP-04、WP-06、WP-07、WP-10 |每个状态、result 和 error branch 的直接 test mapping。
|SYS-EXC-002 |WP-05 |VER-026、VER-027 与 active-line matcher source audit。
|SYS-EXC-003 |WP-04 |VER-006 与 exhaustive state projection compile boundary。
|SYS-EXC-004 |WP-03、WP-06 |VER-011、mutex scope inspection 与 concurrency test。
|SYS-EXC-005 |WP-04、WP-06、WP-10 |VER-033 与四 adapter 并发重复套件。
|SYS-EXC-006 |WP-03、WP-06 |VER-004、VER-016 与 VER-031 的 response-loss fault injection。
|SYS-EXC-007 |WP-07 |VER-020 与全部 byte split position contract suite。
|SYS-EXC-008 |WP-07 |VER-019、VER-024 与 10 MiB mixed-output fixture。
|SYS-EXC-009 |WP-10 |严格 coverage report、排除项清单与 gate exit status。
|SYS-EXC-010 |WP-03、WP-04、WP-06、WP-09 |真实 daemon integration manifest，包含 VER-006、VER-007、VER-008、VER-012、VER-013、VER-017、VER-030、VER-031、VER-032、VER-036、VER-039 与 VER-041。
|SYS-EXC-011 |WP-09、WP-10 |VER-032、VER-041 与 suite 后资源清单审计。
|SYS-EXC-012 |WP-01、WP-11 |changed-code comment audit；每条保留注释均关联不可见约束。
|SYS-EXC-013 |WP-11 |scoped diff、changed-path allowlist 与 release commit inspection。
|SYS-EXC-014 |WP-00、WP-10、WP-11 |本节追踪矩阵、test manifest 与最终 traceability report。
|SYS-EXC-015 |WP-11 |clean-checkout release verification 连续两次运行及 artifact hash comparison。
|===
...
|SYS-DOD-002
|目标 `pueued 4.0.4` 与 exact `pueue-lib 0.31.1` 完成真实 handshake；不使用 CLI wrapper。
...
|SYS-DOD-005
|execution 发布时已经绑定 task identity fingerprint；task ID、secret、environment 和其他 adapter task 不进入公共 schema。
...
|SYS-DOD-009
|最近观察为 queued/stashed 的 cancel 在 Remove success 后进入 cancelled 控制终态且 `output_complete = false`；running/paused cancel 请求 kill 并稍后观察 killed；stashed force-start 与 cancel 的竞态 backend result 不被猜测；其他状态行为明确。
...
|SYS-DOD-014
|Rust workspace TOML 产生 canonical absolute path；Python callback resolver contract 保持有效；两种配置 failure 都阻止 tool ready。
...
|SYS-DOD-017
|设计 `VER-001` 至 `VER-041` 全部具有自动化测试并通过。
...
|SYS-EXC-010
|真实 daemon integration 覆盖 success、所有 failure result、stashed force-start、queued/running/paused control、external removal、disconnect、adapter close task persistence，以及多层 process tree pause/resume/kill 后无存活 descendant。
...
|SYS-EXC-015
|clean checkout release verification 连续执行两次产生相同 tool schema、测试结果、coverage gate 结论和 generated diagram diff。
|===
...
. 是否能对 queued/stashed task 发起删除控制、对 running/paused task 发起进程终止，并区分 cancelled 控制终态与 killed backend result？
