...
* 建立路径、symlink、broken symlink、permission failure、root、marker missing、file/directory mismatch 和 malformed TOML tests。
...
Not done when::
两个 engine 对同一环境变量使用不同 grammar、Rust 自动扫描 `.git`/`.codex`、工具允许逐次 cwd override、symlink path 未规范化，或错误配置静默使用 launch cwd。
...
|SYS-DOD-014
|workspace TOML contract 在 Python 与 Rust engine 中产生相同 canonical absolute path，配置与 canonicalization failure 阻止 tool ready。
