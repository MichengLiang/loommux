...
无参数 `loommux` 使用 Studio 或 MCP host 启动的子进程标准输入输出；`loommux
--server` 在指定 host、port 与 path 上提供 Streamable HTTP endpoint。两种启动形式
只改变 MCP message 的承载方式，不改变一个 server process 只拥有一个 kernel 的边界，
也不改变八工具、workspace、execution、日志或图像投影。

所有无 `--result-mode` 的启动形式都只返回 `content`。`--result-mode structured`
是唯一会返回 `content` 与 `structuredContent` 的显式 opt-in。该选择只改变调用方
可观察的结果通道，不改变同一 execution 的模型可读文本或图像 content。server 不
自动推断客户端能力。

`loommux` 的无参数默认值是 content stdio；`loommux --server` 的默认值是 content
Streamable HTTP，默认 bind 为 `127.0.0.1:8801/mcp`。workspace 在两种启动形式下
都默认使用 launch cwd，只有 host 显式提供 resolver 时才改变。
...
| Output projection 层 | IOPub text 与展示 events。 | stream、combined log 与有序图像内容。 | 终端屏幕渲染。 |
...
每一段文本在写入 `Execution`、`ExecutionLogs` 或 MCP response 之前都必须通过
同一个 terminal text normalizer。任何旁路都违反本契约。
...
`stdout`、`stderr`、`result`、`traceback` 是同一已规范化文本的流投影。
...
都不含 ANSI escape sequence；业务文本仍存在并可搜索；combined 中事件的相对顺序
与 IOPub 到达顺序一致；跨 write sequence 没有控制残片出现在任何公开文本中。
