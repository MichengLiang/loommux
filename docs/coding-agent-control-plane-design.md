...
本文是 workspace 解析、kernel 启动、文本输出和相关黑盒验收的事实源。实现、
...
1. 正常八工具闭环只依赖公开 execution、kernel 状态和文本日志，不依赖用户的
...
本文不定义人类终端模拟、富媒体渲染、多 kernel session、持久 execution
数据库、动态 workspace 切换或宿主插件框架。它们不是八工具闭环成立的条件。
...
| Text projection 层 | IOPub text events。 | stream、combined log、monitor text 的公开文本表面。 | 终端屏幕与富媒体渲染。 |
...
run_python ---------------------------------------------------+
                                                   ...
                                                   IOPub text events
                                                              |
                                                              v
                                               terminal text normalizer
                                                              |
                                                              v
                                      Execution + ExecutionLogs + monitor event
                                                              |
                                                              v
                                   eight MCP tools and their text/status surfaces
```
...
| `src/loommux/kernel_session.py` | 启动 `KernelLaunch`、接收 IOPub、将文本交给 normalizer。 |
| `src/loommux/adapter.py` | execution 生命周期、公开状态与 bootstrap 的集成。 |
| `src/loommux/execution.py` | 已规范化文本写入 execution 和 logs。 |
