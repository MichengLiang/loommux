...
`run_python` 的 freeform 输入、默认 10 秒等待和 `# loommux:` control-directive 语义以 `docs/ipython-mcp-freeform-run-python-design.md` 为事实源。`run_python` docstring 必须使用以下内容：
...
def run_python(freeform: str) -> dict[str, Any]:
    ...
    本次调用默认等待 10 秒。若本次调用需要等待更长时间，使用控制注释::

        # loommux: --wait 120

    可与 ``--full-output`` 组合。无效或重复选项返回
    ``invalid_loommux_directive``，不会创建 execution。等待上限只控制本次
